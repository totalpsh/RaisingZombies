...
public class StageManager : MonoBehaviour, ISaveDataProvider
{
    ...
    public int CurrentStageNumber => GetCurrentStageNumber();
    ...
    private void Awake()
    {
        ...
        if (!saveManager.RegisterProvider(this)) Debug.LogError("[StageManager] Stage Progress Provider 등록에 실패했습니다.", this);
    }
    ...
    private void AdvanceStageProgressAndSave()
    {
        ...
        if (!saved) Debug.LogWarning("[StageManager] Stage Clear 진행도 저장에 실패했습니다. 다음 저장에서 다시 시도합니다.", this);
    }
    ...
    public void RestoreSaveData(object data)
    {
        ...
        if (!TryMigrateStageProgressState(restoredState)) throw new InvalidOperationException("지원하지 않는 Stage Provider 저장 버전입니다.");
        ApplyStageProgressState(restoredState);
        if (storedVersion != CurrentStageSaveVersion || storedStageNumber != GetCurrentStageNumber() || storedCompletion != _allStagesCompleted)
